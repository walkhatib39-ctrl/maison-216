...
<div class="space-y-6">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div class="flex flex-col gap-2 sm:flex-row">
            <a href="{{ route('workshop.clients.index') }}" class="inline-flex items-center justify-center rounded-xl border border-[#d8c7af] bg-white px-4 py-2.5 text-sm font-bold text-[#171411] hover:bg-[#fbf7f0]">Clients</a>
            <a href="{{ route('workshop.orders.index') }}" class="inline-flex items-center justify-center rounded-xl border border-[#d8c7af] bg-white px-4 py-2.5 text-sm font-bold text-[#171411] hover:bg-[#fbf7f0]">Commandes</a>
            <a href="{{ route('workshop.orders.create') }}" class="inline-flex items-center justify-center rounded-xl bg-[#171411] px-4 py-2.5 text-sm font-bold text-white hover:bg-[#a47834]">+ Ajouter commande</a>
        </div>
    </div>
            ...
            <section id="group-{{ $loop->index }}" class="overflow-hidden rounded-2xl border border-[#eadfce] bg-white shadow-sm">
                ...
                <div class="divide-y divide-[#f0e7da]">
                    @forelse($group['orders'] as $order)
                        <a href="{{ route('workshop.orders.show', $order) }}" class="block px-5 py-4 transition hover:bg-[#fbf7f0]">
