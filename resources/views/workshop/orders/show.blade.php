...
<div class="space-y-6">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div class="flex flex-col gap-2 sm:flex-row">
            <a href="{{ route('workshop.orders.edit', $order) }}" class="inline-flex items-center justify-center rounded-xl bg-[#171411] px-4 py-2.5 text-sm font-bold text-white hover:bg-[#a47834]">Modifier</a>
        </div>
    </div>
            ...
            <section class="rounded-2xl border border-[#eadfce] bg-white p-6 shadow-sm">
                ...
                @if($order->client)
                    <div class="mt-5 space-y-3 text-sm">
                        ...
                        <a href="{{ route('workshop.clients.show', $order->client) }}" class="inline-flex rounded-xl border border-[#d8c7af] bg-[#fbf7f0] px-3 py-2 text-xs font-bold text-[#171411] hover:bg-white">Voir fiche client</a>
                    </div>
                @else
        ...
        <div class="space-y-6">
            ...
            <section class="overflow-hidden rounded-2xl border border-[#eadfce] bg-white shadow-sm">
                <div class="flex items-center justify-between gap-3 border-b border-[#eadfce] bg-[#fbf7f0] px-5 py-4">
                    ...
                    <a href="{{ route('workshop.orders.edit', $order) }}" class="text-sm font-bold text-[#8e6322] hover:text-[#171411]">Ajouter</a>
                </div>
                    ...
                    @forelse($order->files as $file)
                        <div class="overflow-hidden rounded-2xl border border-[#eadfce] bg-[#fbf7f0]">
                            ...
                            <div class="p-3">
                                ...
                                <form method="POST" action="{{ route('workshop.orders.files.destroy', [$order, $file]) }}" class="mt-3" onsubmit="return confirm('Supprimer ce fichier ?')">
                                    @csrf
                                    @method('DELETE')
