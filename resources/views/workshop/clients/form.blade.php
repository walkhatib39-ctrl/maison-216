...
<div class="mx-auto max-w-4xl space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
            <a href="{{ route('workshop.clients.index') }}" class="text-sm font-bold text-[#8e6322] hover:text-[#171411]">Retour aux clients</a>
        ...
        @if($isEdit)
            <a href="{{ route('workshop.orders.create', ['client_id' => $client->id]) }}" class="inline-flex items-center justify-center rounded-xl bg-[#171411] px-4 py-2.5 text-sm font-bold text-white transition hover:bg-[#a47834]">+ Commande</a>
        @endif
    </div>
    ...
    <form method="POST" action="{{ $isEdit ? route('workshop.clients.update', $client) : route('workshop.clients.store') }}" class="space-y-6">
        @csrf
        @if($isEdit)
            @method('PUT')
        @endif
        ...
        <div class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            <a href="{{ $isEdit ? route('workshop.clients.show', $client) : route('workshop.clients.index') }}" class="inline-flex items-center justify-center rounded-xl border border-[#d8c7af] bg-white px-5 py-3 text-sm font-bold text-[#171411] hover:bg-[#fbf7f0]">Annuler</a>
