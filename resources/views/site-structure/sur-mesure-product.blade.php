    ...
    <div class="container relative mx-auto grid gap-10 px-4 py-14 lg:grid-cols-[0.92fr_1.08fr] lg:items-center lg:py-20">
        ...
        <div class="relative">
            <div class="rounded-[40px] border border-[#d8c7af] bg-white p-3 shadow-[0_36px_90px_rgba(23,20,17,0.16)]">
                <div class="relative min-h-[360px] overflow-hidden rounded-[30px] bg-cover bg-center lg:min-h-[540px]" style="background-image: linear-gradient(180deg, rgba(23,20,17,0.04), rgba(23,20,17,0.64)), url('{{ $data['heroImage'] }}');">
                    <div class="absolute bottom-6 left-6 right-6 rounded-[26px] border border-white/18 bg-[#171411]/72 p-5 text-white backdrop-blur">
                        <div class="text-xs font-bold uppercase tracking-[0.22em] text-[#d5b170]">Fabrication sur mesure</div>
                        <p class="mt-2 text-sm leading-6 text-white/78">Métré, conception, fabrication atelier, pose et SAV avec un seul interlocuteur.</p>
                    </div>
                </div>
            </div>
        </div>
...
<section id="realisations-sur-mesure" class="bg-white py-16 lg:py-20">
    <div class="container mx-auto px-4">
        ...
        <div class="grid gap-5 md:grid-cols-3">
            @foreach([
                ['type' => $data['name'], 'place' => 'Sur mesure', 'image' => $data['heroImage'], 'copy' => 'Conception, fabrication atelier et pose chez le client.'],
                ['type' => 'Projet villa', 'place' => 'Résidentiel', 'image' => $heroImage, 'copy' => 'Plusieurs aménagements coordonnés dans un même projet.'],
                ['type' => 'Agencement professionnel', 'place' => 'B2B', 'image' => $restaurantImage, 'copy' => 'Mobilier sur mesure pour usage intensif et image de marque.'],
            ] as $realization)
