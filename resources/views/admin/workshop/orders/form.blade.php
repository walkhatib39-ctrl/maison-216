...
<div class="mx-auto max-w-6xl space-y-6">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <a href="{{ route('admin.workshop.clients.create') }}" class="inline-flex items-center justify-center rounded-xl border border-[#d8c7af] bg-white px-4 py-2.5 text-sm font-bold text-[#171411] transition hover:bg-[#fbf7f0]">+ Ajouter client</a>
    </div>
    ...
    <form method="POST" action="{{ $isEdit ? route('admin.workshop.orders.update', $order) : route('admin.workshop.orders.store') }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @if($isEdit)
            @method('PUT')
        @endif
        ...
        <div class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            <a href="{{ $isEdit ? route('admin.workshop.orders.show', $order) : route('admin.workshop.orders.index') }}" class="inline-flex items-center justify-center rounded-xl border border-[#d8c7af] bg-white px-5 py-3 text-sm font-bold text-[#171411] hover:bg-[#fbf7f0]">Annuler</a>
