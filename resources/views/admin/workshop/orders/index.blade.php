...
<div class="space-y-6">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div class="flex flex-col gap-2 sm:flex-row">
            <a href="{{ route('admin.workshop.orders.export', request()->query()) }}" class="inline-flex items-center justify-center rounded-xl border border-[#d8c7af] bg-white px-4 py-2.5 text-sm font-bold text-[#171411] transition hover:bg-[#fbf7f0]">Export Excel</a>
            <a href="{{ route('admin.workshop.clients.index') }}" class="inline-flex items-center justify-center rounded-xl border border-[#d8c7af] bg-white px-4 py-2.5 text-sm font-bold text-[#171411] transition hover:bg-[#fbf7f0]">Clients</a>
            <a href="{{ route('admin.workshop.orders.create') }}" class="inline-flex items-center justify-center rounded-xl bg-[#171411] px-4 py-2.5 text-sm font-bold text-white transition hover:bg-[#a47834]">+ Ajouter commande</a>
        </div>
    </div>
    ...
    <form method="GET" class="rounded-2xl border border-[#eadfce] bg-white p-4 shadow-sm">
        ...
        <div class="mt-4 flex flex-wrap gap-3">
            ...
            <a href="{{ route('admin.workshop.orders.index') }}" class="inline-flex items-center rounded-xl px-3 py-2 text-sm font-bold text-[#8e6322] hover:text-[#171411]">Réinitialiser</a>
        </div>
    </form>
                        ...
                        <tr class="transition hover:bg-[#fbf7f0]">
                            ...
                            <td class="px-5 py-4 text-right">
                                <div class="flex justify-end gap-2">
                                    <a href="{{ route('admin.workshop.orders.show', $order) }}" class="rounded-xl border border-[#d8c7af] bg-white px-3 py-2 text-xs font-bold text-[#171411] hover:bg-[#fbf7f0]">Voir</a>
                                    <a href="{{ route('admin.workshop.orders.edit', $order) }}" class="rounded-xl border border-[#d8c7af] bg-white px-3 py-2 text-xs font-bold text-[#171411] hover:bg-[#fbf7f0]">Modifier</a>
                                    <form method="POST" action="{{ route('admin.workshop.orders.destroy', $order) }}" onsubmit="return confirm('Supprimer cette commande ?')">
                                        @csrf
                                        @method('DELETE')
